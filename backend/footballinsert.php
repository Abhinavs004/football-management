...
if (isset($_POST['addPlayer'])) {
    ...
    $sql = "INSERT INTO player (playerID, age, position, position1, position2, position3, apps, goals, assists, tackles, interceptions, clean_sheets, saves, squad_number, fouls, team_id, yellow_card, red_card, nationality) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

    $stmt = $conn->prepare($sql);
    $stmt->bind_param("iissssiiiiiiiiiiiiss", $playerid, $age,$position, $position1, $position2, $position3, $apps, $goals, $assists, $tackles, $interceptions, $clean_sheets, $saves, $squad_number, $fouls, $team_id, $yellow_card, $red_card, $nationality);
...
if (isset($_POST['addLeague'])) {
    ...
    $sql = "INSERT INTO league (leagueID, number_of_clubs, top_scorer_id, top_assister_id, most_red_card, league_leader_id, best_goal_keeper_id, country) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)";

    $stmt = $conn->prepare($sql);
    $stmt->bind_param("iiiiiiis", $leagueid, $number_of_clubs, $top_scorer_id, $top_assister_id, $most_red_card, $league_leader_id, $best_goal_keeper_id, $country);
...
if (isset($_POST['addClub'])) {
    ...
    $sql = "INSERT INTO club (clubID, number_of_players, goals_scored, clean_sheets, fouls, captain_id, matches_played, wins, losses, draws, yellow_card, red_card, goals_conceded, league_id, stadium) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

    $stmt = $conn->prepare($sql);
    $stmt->bind_param("iiiiiiiiisiiisi", $clubid, $number_of_players, $goals_scored, $clean_sheets, $fouls, $captain_id, $matches_played, $wins, $losses, $draws, $yellow_card, $red_card, $goals_conceded, $league_id, $stadium);
